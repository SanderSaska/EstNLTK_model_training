...
class BertTokens2WordsRewriter(Tagger):
    ...
    def __init__(
        ...
        if not callable(decorator):
            raise TypeError(
                f"(!) decorator should be a callable function, not {type(decorator)}"
            )
...
def map_words_to_bert_tokens(
    words_layer: Layer, bert_tokens_layer: Layer, warn_unmatched: bool = True
):
    """
    ...
    unmatched_bert_tokens = set([j for j in range(len(bert_tokens_layer))])
    word_id = 0
    bert_tokens_start = 0
    while word_id < len(words_layer):
        ...
        while i < len(bert_tokens_layer):
            bert_tokens = bert_tokens_layer[
                i
            ]  # A single token or an enveloping tokens phrase
            if bert_tokens.start < w_end and w_end < bert_tokens.end:
                ...
                bert_tokens.start <= w_end
                and w_start < bert_tokens.end
                and w_end >= bert_tokens.end
            ):
    ...
    if warn_unmatched and len(unmatched_bert_tokens) > 0:
        # Warn about bert unmatched tokens. Skip tokens that are only
        # SentencePiece/BPE boundary marks or whitespace (e.g. '▁'). These
        ...
        for i in sorted(list(unmatched_bert_tokens)):
            ...
            if tok_text.replace("▁", "").strip() == "":
                ...
                continue
            warnings.warn(
                f"(!) No matching {words_layer.name} span for bert token {bert_tokens_layer[i]}."
            )
    return words_to_bert_tokens_map


def group_words_by_bert_tokens(
    words_layer: Layer,
    words_to_bert_tokens_map: Dict[int, Any],
    only_groups_with_bert_tokens: bool = True,
):
    """
        ...
        if word_id in words_to_bert_tokens_map.keys():
            ...
            while word_id2 < len(words_layer):
                ...
                if word_id2 in words_to_bert_tokens_map.keys():
                    ...
                    common_bert_tokens = set(shared_base_bert_tokens).intersection(
                        set(next_bert_base_tokens)
                    )
