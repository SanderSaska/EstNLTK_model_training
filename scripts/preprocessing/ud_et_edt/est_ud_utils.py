...
class EstUDCorrectionsRewriter:
    ...
    def __init__(
        self,
        apply_linguistic_fixes: bool = True,
    ...
    def rewrite(self, record):
        ...
        if xpostag is None:
            if upostag == "CCONJ":
                ...
                upostag == "NOUN" or upostag == "SYM" and record["lemma"] == "%"
            ):  # Temp. fix for AssertionError while reading in Estonian UD EDT treebank
                record["xpostag"] = "S"
                xpostag = record["xpostag"]
            elif upostag == "PUNCT":
                record["xpostag"] = "Z"
...
class EstUDCorrectionsRetagger(AnnotationRewriter):
    ...
    def __init__(
        ...
        rewrite = EstUDCorrectionsRewriter(
            apply_linguistic_fixes=apply_linguistic_fixes,
            verb_ending_type=verb_ending_type,
        ).rewrite
...
def load_ud_file_with_corrections(
    fnm,
    annotation_layer,
    apply_linguistic_fixes=True,
...
def load_ud_file_texts_with_corrections(
    fnm,
    annotation_layer,
    apply_linguistic_fixes=True,
    add_compound_tokens=True,
    ...
    # Rewrite layer with postcorrections
    fixer = EstUDCorrectionsRetagger(
        annotation_layer,
        apply_linguistic_fixes=apply_linguistic_fixes,
        verb_ending_type=verb_ending_type,
    )
