...
def convert_ud_layer_to_reduced_morph_layer(
    text_obj, ud_layer, output_layer, add_layer=True
):
    ...
    assert ud_layer in text_obj.layers, "(!) Layer {!r} missing from: {!r}".format(
        ud_layer, text_obj.layers
    )
    redux_layer = Layer(
        name=output_layer,
...
def parse_ud_morph_redux_attribs(ud_annotation):
    ...
    ud_feats = (
        _split_feats(ud_annotation["feats"])
        if isinstance(ud_annotation["feats"], str)
        else ud_annotation["feats"]
    )
