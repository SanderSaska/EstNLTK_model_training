...
# * https://bitbucket.org/utDigiHum/public/src/8635582194ef1f2dcec53c40abdfa3f29299a067/skriptid/nimeuksuste_m2rgendamine/bert_ner_tagger.py
...
class BertNERMorphTagger(Tagger): # FIXME: Class name
    ...
    def __init__(
        ...
        # TODO: add words_layer param, because BertTokens2WordsRewriter relies on it
        token_level: bool = False,
        ignore_sentence_boundaries: bool = True,
        ambiguous_label_separator: str = '|',
        **kwargs
    ):
        '''Initializes BertNERMorphTagger

        Args:
            model_location (str, optional): Path to model directory.
            output_layer (str, optional): Name of the output named entity annotations layer. Defaults to 'bert_ner_morph'.
            token_level (bool, optional): Tags the text BERT token-level or EstNLTK's word-level. Defaults to False.
            ignore_sentence_boundaries (bool, optional): Ignore sentence boundaries intersecting with NER phrases. Defaults to True.
            ambiguous_label_separator (str, optional): Separator for labels that the model predicted for one token. Defaults to '|'.
        ...
        # TODO: here and elsewhere: calling it NER is confusing, because this is not a NER task. 
        # rename 'ner' --> 'morph' or 'morph_tagging' or 'token_classification'
        ...
        # TODO: rename 'ner_label' --> 'label' or 'morph_label'
        self.output_attributes = ['bert_tokens', 'nerlabel']
        self.ambiguous_label_separator = ambiguous_label_separator

        # Ignore sentence boundaries intersecting with NER phrases
        # TODO: this is not used at all in the code? then remove it
    ...
    def _get_bert_ner_label_predictions( self, input_str ):
        ...
        # TODO: tulevikus võiks proovida siia sisse integreerida seda top_n_tags ennustamist. 
        #       see oleks siis nii, et konstruktorisse tuleb uus parameeter get_top_n_predictions = 1 
        #       ning kui kasutaja seda muudab, siis lisatakse sõnale vastav arv ennustusi + tõenäosused. 
        #       aga see pole praegu esimene prioriteet. 
    ...
    def _make_layer(self, text: Text, layers: MutableMapping[str, Layer], status: dict) -> Layer:
        ...
        #  TODO: see "sentence tokenization can be wrong" kommentaar kehtis BertNERTagger'is vana 
        #  kirjakeele kohta, mille jaoks meil polnud korrektset lausepiiride määrajat. Tänapäeva 
        #  kirjakeelel on olukord parem. 
        ...
        for k, sentence in enumerate( sentences_layer ):
            ...
            for sent_chunk, (chunk_start, chunk_end) in zip(sent_chunks, sent_chunk_indexes):
                ...
                #
                # TODO: get_tokens_with_start_end_indexes() / get_word_subtoken_indexes() kasutamine 
                # oli ad hoc robustne lahendus, mille tingis minumeelest vanem transformersi versioon, 
                # kus oli raske või võimatu kätte saada berti tokenite (start, end) indekseid. 
                # Uus, elegantsem ja töökindlam lahendus peaks olema:  
                #  https://github.com/estnltk/estnltk/blob/devel_1.7/estnltk_neural/estnltk_neural/taggers/ner/estbertner_tagger.py#L131-L146
                # See peaks samamoodi bert token-ite asukohad kätte andma, tuleb talle lihtsalt 
                # sent_chunk ette anda. Ta peaks tagastama samapalju token-eid kui tagastab 
                # ennustaja, nii et indeksitega peaks saama viia kokku token-id ja labelid
                #   
                ...
                for token_id, token_data in enumerate( tokens_with_indexes ):
                    ...
                    annotation = {'bert_tokens' : bert_tokens, 'nerlabel' : single_label }
        ...
        if self.token_level:
            ...
            rewriter = BertTokens2WordsRewriter(bert_tokens_layer=self.output_layer, output_attributes=self.output_attributes, output_layer=self.output_layer, decorator=rewriter_decorator)
...
def rewriter_decorator(text_obj, sharing_words, shared_bert_tokens):
    ...
    # Extract nerlabels from shared_bert_tokens
    nerlabels = [t['nerlabel'][0] for t in shared_bert_tokens]

    # Count occurrences of each nerlabel
    label_counts = collections.Counter(nerlabels)

    # Get the most common nerlabel (returns a list of tuples (label, count))
    most_common_label = label_counts.most_common(1)[0][0]

    return {
        'words': text_obj.text[sharing_words[0].start:sharing_words[-1].end],
        'bert_tokens': [t.text for t in shared_bert_tokens],
        'nerlabel': most_common_label
    }
